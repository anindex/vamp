...
ffi::Error RmsNormImpl(float eps, ffi::Buffer<ffi::F32> x, ffi::ResultBuffer<ffi::F32> y)
{
    ...
    ffi::ResultBuffer<ffi::F32> res)
{
    ...
    {
        res->typed_data()[idx] = ComputeRmsNorm(eps, lastDim, &(x.typed_data()[n]), &(y->typed_data()[n]));
    }
    return ffi::Error::Success();
}
...
void ComputeRmsNormBwd(int64_t size, float res, const float *x, const float *ct_y, float *ct_x)
{
    float ct_res = 0.0f;
    for (int64_t n = 0; n < size; ++n)
    {
...
ffi::Error RmsNormBwdImpl(
    ffi::Buffer<ffi::F32> res,
    ffi::Buffer<ffi::F32> x,
